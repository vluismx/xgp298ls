			...
			if ($Element > 201 && $Element <= 399)
			{
				if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element))
				{
					...
					if ($CanBuildOne && $NotBuilding)
					{
                        ...
                        $maxElement_pn = pretty_number($maxElement); 
                        $PageTable .= "<br><th><a href='javascript:' onclick=\"document.getElementsByName('fmenge[".$Element."]')[0].value = '$maxElement';\">Máx.</a><br><font color=#808080>({$maxElement_pn})</font></th>";
					}
